...
internal class AudioValidator {
    ...
    private static let silenceProbeFrameCount: AVAudioFramePosition = 16_384
    private static let silenceRMSFloor: Float = 1e-5
    ...
    private static func validateWithAVFoundation(url: URL) async -> AudioValidationResult {
        ...
        do {
            ...
            if try isEffectivelySilent(audioFile) {
    ...
    private static func isEffectivelySilent(_ audioFile: AVAudioFile) throws -> Bool {
        let originalFramePosition = audioFile.framePosition
        defer { audioFile.framePosition = originalFramePosition }
        ...
        while audioFile.framePosition < audioFile.length {
            ...
            let frameCount = AVAudioFrameCount(min(remainingFrames, silenceProbeFrameCount))
            guard
            ...
            if bufferHasAudibleSignal(buffer) {
                return false
            }
        }
    ...
    private static func bufferHasAudibleSignal(_ buffer: AVAudioPCMBuffer) -> Bool {
        ...
        for channelIndex in 0..<channelCount {
            ...
            for frameIndex in 0..<frameCount {
                sumSquares += samples[frameIndex] * samples[frameIndex]
            }
...
internal enum AudioValidationError: LocalizedError {
    case fileNotFound
    case emptyFile
