...
internal class ParakeetService {
    private let logger = Logger(subsystem: "com.whisp.app", category: "ParakeetService")
    ...
    private func loadAudio(url: URL, samplingRate: Int) throws -> [Float] {
        ...
        status = ExtAudioFileGetProperty(
            extFile, kExtAudioFileProperty_FileDataFormat, &propertySize, &fileFormat)
        ...
        status = ExtAudioFileGetProperty(
            extFile, kExtAudioFileProperty_FileLengthFrames, &propertySize, &fileLengthFrames)
        guard status == noErr else {
        ...
        status = ExtAudioFileSetProperty(
            extFile, kExtAudioFileProperty_ClientDataFormat, propertySize, &clientFormat)
        guard status == noErr else {
    ...
    func validateSetup(pythonPath _: String? = nil) async throws {
        ...
        do {
            ...
            throw ParakeetError.transcriptionFailed(
                "Parakeet daemon unavailable: \(error.localizedDescription)")
        }
    }
}
