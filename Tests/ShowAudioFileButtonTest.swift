...
final class ShowAudioFileButtonTest: XCTestCase {
    ...
    func testShowAudioFileButtonOnlyForTranscriptionErrors() {
        ...
        let testCases = [
            ...
            ("No speech detected.", false),
            ("Recording too short.", false),
        ...
        for (errorMessage, shouldBeTranscriptionError) in testCases {
            ...
            XCTAssertEqual(
                isTranscriptionError, shouldBeTranscriptionError,
    ...
    func testTranscriptionErrorGetsThirdButton() {
        ...
        let errorMessages = [
            ...
            "No speech detected.",  // Should get: OK (1 button)
            "Recording too short.",  // Should get: OK (1 button)
            "Generic error",  // Should get: OK (1 button)
        ]

        let expectedButtonCounts = [3, 2, 2, 2, 1, 1, 1]  // Transcription error gets 3 buttons, others get 1-2
        ...
        for (index, errorMessage) in errorMessages.enumerated() {
            ...
            XCTAssertEqual(
                calculatedButtonCount, expectedButtonCount,
    ...
    func testThirdButtonOnlyForTranscriptionErrors() {
        ...
        let testCases = [
            ...
            ("No speech detected.", nil, false),
            ("Recording too short.", nil, false),
        ...
        for (errorMessage, expectedErrorType, shouldHaveThirdButton) in testCases {
            let errorType = classifyError(errorMessage)
            XCTAssertEqual(
                errorType, expectedErrorType, "Error type classification failed for: \(errorMessage)")

            // Test third button logic: should only be true for transcription errors
            let hasThirdButton = (errorType == "transcription")
            XCTAssertEqual(
                hasThirdButton, shouldHaveThirdButton,
