...
class SpeechToTextServiceTests: XCTestCase {
    ...
    func testSpeechToTextErrorDescriptions() {
        ...
        let invalidAudioError = SpeechToTextError.invalidAudio("Audio file not found")
        XCTAssertEqual(invalidAudioError.errorDescription, "Audio file not found")
        ...
        XCTAssertEqual(noSpeechDetectedError.errorDescription, LocalizedStrings.Errors.noSpeechDetected)
    }
    ...
    func testTranscribeDefaultOverloadMapsSilentAudioToNoSpeechDetected() async throws {
        let silentAudioURL = try createSilentAudioFile()
        defer { try? FileManager.default.removeItem(at: silentAudioURL) }

        do {
            _ = try await service.transcribe(audioURL: silentAudioURL)
            XCTFail("Expected noSpeechDetected for silent audio")
        } catch let error as SpeechToTextError {
...
extension SpeechToTextServiceTests {
    private func createTestAudioFile() -> URL {
        ...
        let audioURL = tempDir.appendingPathComponent("test_audio-\(UUID().uuidString).wav")
    ...
    private func createSilentAudioFile() throws -> URL {
        let tempDir = FileManager.default.temporaryDirectory
        let audioURL = tempDir.appendingPathComponent("silent_audio-\(UUID().uuidString).wav")

        guard let format = AVAudioFormat(standardFormatWithSampleRate: 44_100, channels: 1) else {
            throw NSError(
                domain: "SpeechToTextServiceTests",
                code: 1,
    ...
    func testTranscribeProviderMapsSilentAudioToNoSpeechDetected() async throws {
        let silentAudioURL = try createSilentAudioFile()
        defer { try? FileManager.default.removeItem(at: silentAudioURL) }

        do {
            _ = try await service.transcribe(audioURL: silentAudioURL, provider: .openai)
            XCTFail("Expected noSpeechDetected for silent audio")
        } catch let error as SpeechToTextError {
            XCTAssertEqual(error, .noSpeechDetected)
        } catch {
