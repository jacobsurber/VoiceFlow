...
class TranscriptionTypesTests: XCTestCase {
    ...
    func testWhisperModelDownloadURLs() {
        ...
        XCTAssertEqual(
            WhisperModel.largeTurbo.downloadURL.absoluteString, "\(baseURL)/ggml-large-v3-turbo.bin")
    }
    ...
    func testFileSizesContainUnits() {
        ...
        for model in WhisperModel.allCases {
            ...
            XCTAssertTrue(
                size.contains("MB") || size.contains("GB"), "File size for \(model) should contain MB or GB")
        }
    }
    ...
    func testDisplayNamesContainSizeInfo() {
        for model in WhisperModel.allCases {
            ...
            XCTAssertTrue(
                displayName.contains("MB") || displayName.contains("GB"),
    ...
    func testDescriptionsContainUsefulInfo() {
        ...
        for model in WhisperModel.allCases {
            ...
            let hasSpeedInfo =
                description.contains("fast") || description.contains("slow") || description.contains("speed")
            let hasAccuracyInfo = description.contains("accuracy") || description.contains("accurate")
            XCTAssertTrue(
                hasSpeedInfo || hasAccuracyInfo,
